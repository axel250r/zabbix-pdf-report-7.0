...
if (!defined('APP_TMP')) {
  ...
  @is_dir(APP_TMP) || @mkdir(APP_TMP, 0770, true);
}

// Simple trace
function ztrace_login($m){ @file_put_contents(APP_TMP.'/zbx_login_trace.log','['.date('Y-m-d H:i:s').'] '.$m."\n",FILE_APPEND); }

// API validation (user typed by human)
function api_validate_login(string $user, string $pass, ?string &$err=null): bool {
  $payload = json_encode(['jsonrpc'=>'2.0','method'=>'user.login','params'=>['username'=>$user,'password'=>$pass],'id'=>1,'auth'=>null]);
...
function web_login(string $user, string $pass, string $cookieJar, ?string &$err=null): bool {
  ...
  // warmup
  $ch=curl_init($base.'/index.php'); curl_setopt_array($ch,$opt); $html=curl_exec($ch); curl_close($ch);
  $sid=''; if ($html && preg_match('/name="sid"\\s+value="([^"]*)"/i',$html,$m)) $sid=$m[1];
  $form_refresh='1'; if ($html && preg_match('/name="form_refresh"\\s+value="([^"]*)"/i',$html,$m)) $form_refresh=$m[1];

  // post
  $post=['name'=>$loginUser,'password'=>$pass,'autologin'=>1,'enter'=>'Sign in','form_refresh'=>$form_refresh];
  if ($sid!=='') $post['sid']=$sid;
  $ch=curl_init($base.'/index.php'); curl_setopt_array($ch,$opt+[CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>http_build_query($post),CURLOPT_REFERER=>$base.'/index.php',CURLOPT_HTTPHEADER=>['Content-Type: application/x-www-form-urlencoded']]); $resp=curl_exec($ch); curl_close($ch);
...
if ($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if ($u==='' || $p===''){ $msg=t('login_error_invalid_form'); }
    ...
    if (!api_validate_login($u,$p,$apiErr)){
      ...
      $cookieJar = APP_TMP.DIRECTORY_SEPARATOR.'cj_'.bin2hex(random_bytes(6)).'.txt';
...
<div class="wrap">
  <form class="card" method="post" action="login.php" autocomplete="off">
    ...
    <?php if ($msg): ?><div class="err"><?php echo htmlspecialchars($msg,ENT_QUOTES,'UTF-8'); ?></div><?php endif; ?>
